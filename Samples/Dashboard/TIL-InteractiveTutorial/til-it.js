...
(function () {
  ...
  $(document).ready(function () {
    ...
    api.initializeAsync().then(async function () {
      ...
      $("#load-first-sheet").on("click", handleAddSheetButtonClick);
      $("#sex-to-rows").on("click", handleAddSexToRowsClick);
  ...
  async function handleAddSheetButtonClick() {
    const commandNamespace = 'tabdoc';
    const sheets = await api.workbook.executeCommandAsync(commandNamespace, 'get-sheets-info', {});
    console.log(sheets);
    const command = await api.workbook.executeCommandAsync(commandNamespace, 'add-sheet-to-dashboard', {
  ...
  */
  ...
  async function handleAddSexToRowsClick() {
    ...
    await api.workbook.executeCommandAsync("tabdoc", 'set-active-zone', {
      "skip-if-active": "true",
      "dashboard": "Tutorial",
      "zone-id": "8"
    })
    const command = await api.workbook.executeCommandAsync("tabdoc", 'drop-on-shelf', addPillToSheet(
      "[federated.0okqa6q0d7n9uc16bo76i0aq6p3p].[none:ISO:nk]",
      "rows",
      0,
